...
if Anisotropy_Option:
    ...
    if OMDOT_option:
        # OMDOT = bilby.core.prior.analytical.Normal(pars['OMDOT'], 2, 'OMDOT')
        OMDOT = bilby.core.prior.Uniform(10, 25, 'OMDOT')
    else:
        OMDOT = bilby.core.prior.analytical.DeltaFunction(pars['OMDOT'], name='OMDOT')
    if OM_option:
        # OM = bilby.core.prior.analytical.Normal(pars['OM'], 5, 'OM', boundary='periodic')
        OM = bilby.core.prior.Uniform(0, 360, 'OM', boundary='periodic')
    else:
        OM = bilby.core.prior.analytical.DeltaFunction(pars['OM'], name='OM')
    if TAUEFAC_option:
        TAUEFAC = bilby.core.prior.Uniform(-10, 10, 'TAUEFAC')  # -1, 1
    else:
    ...
    if DNUEFAC_option:
        DNUEFAC = bilby.core.prior.Uniform(-10, 10, 'DNUEFAC')  # -1, 1
    else:
    ...
    priors = dict(d=d, s=s, k=k, A=A, vism_ra=vism_ra, vism_dec=vism_dec,
                  KIN=KIN, KOM=KOM, OM=OM, OMDOT=OMDOT, psi=psi, R=R, alpha=alpha,
