...
outfile = str(outdir) + str(psrname) + '_ScintillationResults'
...
if measure:
    ...
    for dynspec in dynspecs:
        ...
        if dyn.freq > 1000:
            ...
            f_high = int((f_max + 1650)/2)
            bw_high = int(f_max - 1650)
            for istart_f in range(1650, f_max, int(10)):
...
if model:
    ...
    plt.errorbar(phase, viss, yerr=visserr, fmt=' ', ecolor='C0', alpha=0.4,
                 elinewidth=5)
    plt.xlabel('Orbital phase (degrees)')
    plt.ylabel('Viss (km/s)')
    plt.xlim(0, 360)
    plt.title(psrname + ' Scintillation velocity')
    ax.legend(fontsize='xx-small')
    plt.savefig(plotdir + str(psrname) + "_Viss_OrbitalPhase_Test.png")
    plt.show()
    plt.close()
