...
labellist = {
    "Anisotropic_Global2": Anisotropy_Option,
    "Isotropic": Isotropic,
    "Flipped": sense,
...
OMDOT = pars['OMDOT']
OM_1 = pars['OM']
OM_2 = OM_1 + OMDOT * ((T0_2 - T0_1)/(365.25))
OM_2 = OM_2 % 360
pars['T0'] = T0_3
pars['OM'] = OM_2
...
if mean_option:
    alpha = 3.1631191270636885

    dnu_epochs = [0.06865494253529988,
                  0.0765555985519756,
                  0.08062186370216701,
                  0.08869145055970389,
                  0.10686510657588129,
    ...
    for i in range(len(epochs)):
        ...
        dnu[inds] = dnu_epochs[i] * (freqMHz[inds] / 1000) ** alpha
...
viss = viss[new_sort].flatten()
visserr = visserr[new_sort].flatten()
mjd = mjd[new_sort].flatten()
freqMHz = freqMHz[new_sort].flatten()
freqGHz = freqMHz / 1e3
dnu = dnu[new_sort].flatten()
dnuerr = dnuerr[new_sort].flatten()
tau = tau[new_sort].flatten()
tauerr = tauerr[new_sort].flatten()
phase = phase[new_sort].flatten()
U = U[new_sort].flatten()
ve_ra = ve_ra[new_sort].flatten()
ve_dec = ve_dec[new_sort].flatten()
...
def effective_velocity_annual_bilby(
    xdata, d, s, k, A, vism_ra, vism_dec, KIN, KOM, OM1, OM2, OM4, OM5, OM6, OM7, OMDOT,
    TAUEFAC, DNUEFAC, TAUESKEW, DNUESKEW, alpha, psi, R, **kwargs):
    """
    ...
    # params_['vism_ra2']  = vism_ra2           # (km s-1)
    # params_['vism_dec2'] = vism_dec2          # (km s-1)
    # params_['vism_ra3']  = vism_ra3           # (km s-1)
    # params_['vism_dec3'] = vism_dec3          # (km s-1)
    # params_['vism_ra4']  = vism_ra4           # (km s-1)
    # params_['vism_dec4'] = vism_dec4          # (km s-1)
    # params_['vism_ra5']  = vism_ra5           # (km s-1)
    # params_['vism_dec5'] = vism_dec5          # (km s-1)
    # params_['vism_ra6']  = vism_ra6           # (km s-1)
    # params_['vism_dec6'] = vism_dec6          # (km s-1)
    # params_['vism_ra7']  = vism_ra7           # (km s-1)
    # params_['vism_dec7'] = vism_dec7          # (km s-1)
    #
    if psi != 500:
        #
        params_['psi']  = psi                  # (deg)
        params_['R']    = R
        # params_['psi2']  = psi2              # (deg)
        # params_['R2']    = R2
        # params_['psi3']  = psi3               # (deg)
        # params_['R3']    = R3
        # params_['psi4']  = psi4               # (deg)
        # params_['R4']    = R4
        # params_['psi5']  = psi5               # (deg)
        # params_['R5']    = R5
        # params_['psi6']  = psi6              # (deg)
        # params_['R6']    = R6
        # params_['psi7']  = psi7               # (deg)
        # params_['R7']    = R7
        #
    params_['KOM']      = KOM               # (deg)
    params_['KIN']      = KIN               # (deg)
    ...
    #
    params_['OM1']      = OM1              # longitude of periastron (deg)
    params_['OM2']      = OM2               # longitude of periastron (deg)
    # params_['OM3']      = OM3               # longitude of periastron (deg)
    params_['OM4']      = OM4               # longitude of periastron (deg)
    params_['OM5']      = OM5               # longitude of periastron (deg)
    params_['OM6']      = OM6               # longitude of periastron (deg)
    params_['OM7']      = OM7               # longitude of periastron (deg)
    #
    ...
    # vism_ra[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] = vism_ra[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] * vism_ra1
    # vism_ra[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] = vism_ra[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] * vism_ra2
    # vism_ra[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] = vism_ra[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] * vism_ra3
    # vism_ra[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] = vism_ra[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] * vism_ra4
    # vism_ra[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] = vism_ra[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] * vism_ra5
    # vism_ra[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] = vism_ra[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] * vism_ra6
    # vism_ra[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] = vism_ra[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] * vism_ra7
    
    # # vism_dec needs to be an array that has values of vism_dec1 for mjd epoch 1
    # vism_dec = np.ones((np.shape(mjd)))
    # vism_dec[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] = vism_dec[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] * vism_dec1
    # vism_dec[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] = vism_dec[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] * vism_dec2
    # vism_dec[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] = vism_dec[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] * vism_dec3
    # vism_dec[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] = vism_dec[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] * vism_dec4
    # vism_dec[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] = vism_dec[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] * vism_dec5
    # vism_dec[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] = vism_dec[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] * vism_dec6
    # vism_dec[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] = vism_dec[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] * vism_dec7
    
    # # psi needs to be an array that has values of psi1 for mjd epoch 1
    # psi = np.ones((np.shape(mjd)))
    # psi[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] = psi[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] * psi1
    # psi[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] = psi[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] * psi2
    # psi[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] = psi[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] * psi3
    # psi[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] = psi[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] * psi4
    # psi[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] = psi[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] * psi5
    # psi[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] = psi[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] * psi6
    # psi[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] = psi[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] * psi7
    
    # # R needs to be an array that has values of R1 for mjd epoch 1
    # R = np.ones((np.shape(mjd)))
    # R[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] = R[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] * R1
    # R[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] = R[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] * R2
    # R[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] = R[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] * R3
    # R[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] = R[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] * R4
    # R[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] = R[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] * R5
    # R[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] = R[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] * R6
    # R[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] = R[np.argwhere((mjd>mjd_flor[6])*(mjd<mjd_ciel[6]))] * R7

    mjd_ciel = [59780, 59800, 59880, 59910, 59950, 70000]
    mjd_flor = [0,     59780, 59870, 59900, 59940, 59990]
    
    # OM needs to be an array that has values of OM1 for mjd epoch 1
    OM = np.ones((np.shape(mjd)))
    OM[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] = OM[np.argwhere((mjd>mjd_flor[0])*(mjd<mjd_ciel[0]))] * OM1
    OM[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] = OM[np.argwhere((mjd>mjd_flor[1])*(mjd<mjd_ciel[1]))] * OM2
    # OM[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] = OM[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] * OM3
    OM[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] = OM[np.argwhere((mjd>mjd_flor[2])*(mjd<mjd_ciel[2]))] * OM4
    OM[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] = OM[np.argwhere((mjd>mjd_flor[3])*(mjd<mjd_ciel[3]))] * OM5
    OM[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] = OM[np.argwhere((mjd>mjd_flor[4])*(mjd<mjd_ciel[4]))] * OM6
    OM[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] = OM[np.argwhere((mjd>mjd_flor[5])*(mjd<mjd_ciel[5]))] * OM7
    ...
    OM1   = params_['OM1']   * np.pi/180    # longitude of periastron (rad)
    OM2   = params_['OM2']   * np.pi/180    # longitude of periastron (rad)
    # OM3   = params_['OM3']   * np.pi/180    # longitude of periastron (rad)
    OM4   = params_['OM4']   * np.pi/180    # longitude of periastron (rad)
    OM5   = params_['OM5']   * np.pi/180    # longitude of periastron (rad)
    OM6   = params_['OM6']   * np.pi/180    # longitude of periastron (rad)
    OM7   = params_['OM7']   * np.pi/180    # longitude of periastron (rad)
...
if Anisotropy_Option:
    ...
    if vism_ra_option:
        ...
        # vism_ra3 = bilby.core.prior.Uniform(-300, 300, 'vism_ra3')
        # vism_ra4 = bilby.core.prior.Uniform(-300, 300, 'vism_ra4')
        # vism_ra5 = bilby.core.prior.Uniform(-300, 300, 'vism_ra5')
        # vism_ra6 = bilby.core.prior.Uniform(-300, 300, 'vism_ra6')
        # vism_ra7 = bilby.core.prior.Uniform(-300, 300, 'vism_ra7')
    else:
    ...
    if vism_dec_option:
        ...
        # vism_dec3 = bilby.core.prior.Uniform(-300, 300, 'vism_dec3')
        # vism_dec4 = bilby.core.prior.Uniform(-300, 300, 'vism_dec4')
        # vism_dec5 = bilby.core.prior.Uniform(-300, 300, 'vism_dec5')
        # vism_dec6 = bilby.core.prior.Uniform(-300, 300, 'vism_dec6')
        # vism_dec7 = bilby.core.prior.Uniform(-300, 300, 'vism_dec7')
    else:
    ...
    # TAUEFAC, DNUEFAC, TAUESKEW, DNUESKEW, alpha, psi1, psi2, psi3, psi4, psi5,
    # psi6, psi7, R1, R2, R3, R4, R5, R6, R7, **kwargs

    priors = dict(d=d, s=s, k=k, A=A, vism_ra=vism_ra, vism_dec=vism_dec,
                  KIN=KIN, KOM=KOM, OM1=OM1, OM2=OM2, OM4=OM4, OM5=OM5,
                  OM6=OM6, OM7=OM7, OMDOT=OMDOT, alpha=alpha, psi=psi,
                  R=R, TAUEFAC=TAUEFAC, DNUEFAC=DNUEFAC, TAUESKEW=TAUESKEW,
                  DNUESKEW=DNUESKEW)
...
# result = bilby.result.read_in_result(filename="/Users/jacobaskew/Desktop/DoublePulsar_Project/0737-3039A/New/Modelling/Anisotropic/Anisotropic_Global/Anisotropic_Global_result.json")
...
#
OM1_result = result.posterior['OM1'][NUM]
OM1err_result = np.std(result.posterior['OM1'].values)
params.add('OM1', value=OM1_result, vary=False)
params.add('OM1err', value=OM1err_result, vary=False)
...
orig_veff = effective_velocity_annual_bilby(
    ...
    vism_dec_result, KIN_result, KOM_result, OM1_result, OM2_result, OM4_result,
    OM5_result, OM6_result, OM7_result, OMDOT_result,
...
orig_veff_VE = effective_velocity_annual_bilby(
    ...
    0, KIN_result, KOM_result, OM1_result, OM2_result, OM4_result,
    OM5_result, OM6_result, OM7_result, OMDOT_result,
    TAUEFAC_result, DNUEFAC_result, TAUESKEW_result, DNUESKEW_result,
    alpha_result, psi_result, R_result, **kwargs)
...
Model_veff = effective_velocity_annual_bilby(
    Model_mjd, d_result, s_result, k_result, A_result, vism_ra_result,
    vism_dec_result, KIN_result, KOM_result, OM1_result, OM2_result, OM4_result,
    OM5_result, OM6_result, OM7_result, OMDOT_result,
    TAUEFAC_result, DNUEFAC_result, TAUESKEW_result, DNUESKEW_result,
    alpha_result, psi_result, R_result, **kwargs)
...
# mjds = np.delete(mjds, 2)
OMs    = np.asarray([OM1_result, OM2_result, OM4_result, OM5_result, OM6_result, OM7_result])
# for i in range(0, len(OMs)):
#     if OMs[i] > 180:
#         OMs[i] = OMs[i] - 180
OMerrs = np.asarray([OM1err_result, OM2err_result, OM4err_result, OM5err_result, OM6err_result, OM7err_result])
